...
import { Search, Landmark, Mic2, Music2, ListMusic, Feather, Globe, ChevronLeft, ChevronRight, X, Library } from 'lucide-react';
import { PERIODS, INSTRUMENTS, GENRES, REGIONS } from '../constants';
import { Composer } from '../types';

type Category = 'Periods' | 'Instruments' | 'Genres' | 'Playlists' | 'Composers' | 'Regions';

const CATEGORIES: { label: Category; icon: React.FC<any> }[] = [
  { label: 'Periods', icon: Landmark },
  { label: 'Instruments', icon: Mic2 },
  { label: 'Genres', icon: Music2 },
  { label: 'Playlists', icon: ListMusic },
  { label: 'Composers', icon: Feather },
...
export const SearchScreen: React.FC<SearchScreenProps> = ({ composers }) => {
  ...
  const filteredComposers = searchQuery 
    ? composers.filter(c => c.name.toLowerCase().includes(searchQuery.toLowerCase()))
    : composers;
  ...
  const renderSearchHeader = () => (
    <>
       <header className="px-4 pt-12 pb-2">
        <h1 className="text-4xl font-bold tracking-tight text-textMain font-serif">Search</h1>
      </header>
  ...
  const renderComposersList = () => (
    <div className="bg-background min-h-screen pb-24">
      {renderCategoryHeader('Composers')}
      <div className="divide-y divide-gray-100 pl-4">
        {composers.length === 0 ? (
          <div className="flex flex-col items-center justify-center pt-20 pr-4">
             ...
             <p className="text-lg font-serif font-bold text-textMain">No Composers Added</p>
             <p className="text-sm text-textSub mt-1">Upload composer info to see them here.</p>
          </div>
        ) : (
  ...
  const renderSimpleList = (title: string, items: string[]) => (
    <div className="bg-background min-h-screen pb-24">
      ...
      <div className="divide-y divide-gray-100 pl-4">
        {items.map((item) => (
          <div key={item} className="flex items-center justify-between py-4 pr-4 cursor-pointer hover:bg-gray-50 transition-colors active:bg-gray-100">
  ...
  const renderPeriodsList = () => (
    <div className="bg-background min-h-screen pb-24">
      {renderCategoryHeader('Periods')}
      <div className="divide-y divide-gray-100 pl-4">
        {PERIODS.map((period) => (
          <div key={period.name} className="flex items-center justify-between py-4 pr-4 cursor-pointer hover:bg-gray-50 transition-colors active:bg-gray-100">
            <div>
  ...
  if (activeCategory === 'Instruments') return renderSimpleList('Instruments', INSTRUMENTS);
  if (activeCategory === 'Genres') return renderSimpleList('Genres', GENRES);
  if (activeCategory === 'Regions') return renderSimpleList('Regions', REGIONS);
  if (activeCategory === 'Playlists') return renderSimpleList('Playlists', ['Essentials', 'Focus', 'Sleep', 'Piano Masterpieces', 'Violin Concertos']);
      ...
      <div className="px-4 py-4">
        {searchQuery ? (
           ...
           <div className="animate-in fade-in slide-in-from-bottom-2 duration-200">
              <h2 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3 pl-1">Top Results</h2>
                  ...
                  filteredComposers.map(c => (
                    ...
                    >
                       ...
                       <div className="flex-1">
                          ...
                          <p className="text-xs text-textSub">Composer</p>
                       </div>
           ...
           <div className="grid grid-cols-2 gap-4 animate-in fade-in duration-300">
            {CATEGORIES.map((cat) => (
                ...
                <cat.icon 
                    ...
                    strokeWidth={1.5}
                />
                <span className="text-lg font-semibold leading-tight text-textMain font-sans">{cat.label}</span>
                </button>
            ))}
            </div>
        )}
